...
ALU_Ops decode (const ALU_Type data) { //complete
  if (data.instr.lsli.op == ALU_LSLI_OP) { //complete
    // complete
	if (opts.instrs) {
  ...
  else if (data.instr.addr.op == ALU_ADDR_OP) { //complete
	// complete
    if (opts.instrs) { 
  ...
  else if (data.instr.subr.op == ALU_SUBR_OP) { //complete
  ...
  else if (data.instr.add3i.op == ALU_ADD3I_OP) { //complete
    // complete
    if (opts.instrs) { 
  ...
  else if (data.instr.sub3i.op == ALU_SUB3I_OP) { //complete
    // 315: insert code here to print subs instruction
	if (opts.instrs) { 
  ...
  else if (data.instr.add8i.op == ALU_ADD8I_OP) { //complete
    // complete
    if (opts.instrs) { 
  ...
  else if (data.instr.sub8i.op == ALU_SUB8I_OP) { //complete
    // 315: insert code here to print subs instruction
	if (opts.instrs) { 
  ...
  else if (data.instr.cmp.op == ALU_CMP_OP) {  //complete
    // complete
    if (opts.instrs) { 
  ...
  else if (data.instr.mov.op == ALU_MOV_OP) {  //complete
    // complete
    if (opts.instrs) { 
...
DP_Ops decode (const DP_Type data) { //complete
...
SP_Ops decode (const SP_Type data) { //complete
  if (data.instr.mov.op == 2) { //complete
    if (opts.instrs) { 
      cout << "mov";
      if (data.instr.mov.d) {
  ...
  else if (data.instr.add.op == 0) { //complete
    // complete
	if (opts.instrs) { 
      cout << "add";
      if (data.instr.mov.d) {
  ...
  else if (data.instr.cmp.op == 1) { //complete
...
LD_ST_Ops decode (const LD_ST_Type data) { 
  if (data.instr.class_type.opA == LD_ST_REG_OPA) { //complete
    if (data.instr.class_type.opB == LD_ST_OPB_LDRB) { //complete
    ...
    else if (data.instr.class_type.opB == LD_ST_OPB_STRB) { //complete
    ...
    else if (data.instr.class_type.opB == LD_ST_OPB_LDR) { //complete
    ...
    else if (data.instr.class_type.opB == LD_ST_OPB_STR) { //complete
  ...
  else if (data.instr.class_type.opA == LD_ST_IMM_OPA) { //complete
    // complete
  ...
  else if (data.instr.class_type.opA == LD_ST_IMMB_OPA) { //complete
  ...
  else if (data.instr.class_type.opA == LD_ST_IMMSP_OPA) { //complete
    // complete
...
MISC_Ops decode (const MISC_Type data) { //complete
  if (data.instr.push.op == MISC_PUSH_OP) { //complete
    // Push is complete
    if (opts.instrs) { 
      bool multiple = FALSE;
      cout << "push ";
  ...
  else if (data.instr.pop.op == MISC_POP_OP) { //complete
    // Pop is complete
    if (opts.instrs) { 
      bool multiple = FALSE;
      cout << "pop ";
  ...
  else if (data.instr.sub.op == MISC_SUB_OP) { //complete
    // complete
    if (opts.instrs) { 
      cout << "sub sp, #";
  ...
  else if (data.instr.add.op == MISC_ADD_OP) { //complete
    // complete
    if (opts.instrs) { 
      cout << "add sp, #";
  ...
  else if (data.instr.class_type.type_check == ADD_SP8I_TYPE) { //complete
    // complete
    if (opts.instrs) { 
...
int decode (const COND_Type data) { //complete
  // complete
  if (opts.instrs) { 
    cout << "b";
...
int decode (const UNCOND_Type data) { //complete
  // complete
  if (opts.instrs) { 
...
BL_Ops decode (const BL_Type data) { //complete
  // complete
  if (opts.instrs) { 
...
int decode (const LDM_Type data) { //complete
...
int decode (const STM_Type data) { //complete
...
int decode (const LDRL_Type data) { //complete
  // complete
  if (opts.instrs) {
...
int decode (const ADD_SP_Type data) { //complete
  // complete
  if (opts.instrs) { 
